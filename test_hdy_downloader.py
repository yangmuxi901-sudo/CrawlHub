...
# 导入被测模块
from standalone_hdy_downloader import (
    Logger,
    clean_filename,
    clean_html_tags,
    save_qa_file,
...
class TestLogger(unittest.TestCase):
    ...
    def setUp(self):
        self.test_log_path = os.path.join(BASE_DIR, "data", "test_log.txt")
    ...
    def test_log_write(self):
        ...
        with open(self.test_log_path, "r", encoding="utf-8") as f:
            content = f.read()
        self.assertIn("[HDY] [INFO] 测试消息", content)
    ...
    def test_log_level(self):
        ...
        with open(self.test_log_path, "r", encoding="utf-8") as f:
            content = f.read()
        self.assertIn("[HDY] [ERROR] 错误消息", content)
        self.assertIn("[HDY] [WARNING] 警告消息", content)
...
class TestSaveQaFile(unittest.TestCase):
    ...
    def setUp(self):
        self.test_dir = os.path.join(BASE_DIR, "data", "test_qa")
...
class TestDatabaseOperations(unittest.TestCase):
    ...
    def setUp(self):
        self.test_db = os.path.join(BASE_DIR, "data", "test_hdy_sync_record.db")
        # 创建测试数据库
    ...
    def test_get_default_date(self):
        ...
        # 确保数据库中不存在该股票
        conn = sqlite3.connect(self.test_db)
        conn.execute("DELETE FROM hdy_history WHERE ticker = ?", ("sz.999999",))
        conn.commit()
        conn.close()

        # 临时修改 DB_PATH
        import standalone_hdy_downloader as hdy
        orig = hdy.DB_PATH
        hdy.DB_PATH = self.test_db

        date = get_last_question_date("sz.999999")
...
class TestModuleConstants(unittest.TestCase):
    ...
    def test_headers_defined(self):
        """测试请求头定义"""
        from standalone_hdy_downloader import HEADERS
