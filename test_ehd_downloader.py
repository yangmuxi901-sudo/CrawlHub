...
# 导入被测模块
from standalone_ehd_downloader import (
    Logger,
    clean_filename,
    clean_html_tags,
    save_qa_file,
...
class TestLogger(unittest.TestCase):
    ...
    def setUp(self):
        self.test_log_path = os.path.join(BASE_DIR, "data", "test_ehd_log.txt")
    ...
    def test_log_write(self):
        ...
        with open(self.test_log_path, "r", encoding="utf-8") as f:
            content = f.read()
        self.assertIn("[EHD] [INFO] 测试消息", content)
    ...
    def test_log_level(self):
        ...
        with open(self.test_log_path, "r", encoding="utf-8") as f:
            content = f.read()
        self.assertIn("[EHD] [ERROR] 错误消息", content)
        self.assertIn("[EHD] [WARNING] 警告消息", content)
...
class TestSaveQaFile(unittest.TestCase):
    ...
    def setUp(self):
        self.test_dir = os.path.join(BASE_DIR, "data", "test_ehd_qa")
    ...
    def test_save_file(self):
        ...
        result = save_qa_file(self.test_file, question, answer, publish_date)
    ...
    def test_file_content(self):
        ...
        save_qa_file(self.test_file, question, answer, publish_date)
...
class TestDatabaseOperations(unittest.TestCase):
    ...
    def setUp(self):
        self.test_db = os.path.join(BASE_DIR, "data", "test_ehd_sync_record.db")
        # 创建测试数据库
    ...
    def test_get_default_date(self):
        ...
        # 确保数据库中不存在该股票
        conn = sqlite3.connect(self.test_db)
        conn.execute("DELETE FROM ehd_history WHERE ticker = ?", ("sh.999999",))
        conn.commit()
        conn.close()

        # 临时修改 DB_PATH
        import standalone_ehd_downloader as ehd
        orig = ehd.DB_PATH
        ehd.DB_PATH = self.test_db

        date = get_last_question_date("sh.999999")
    ...
    def test_update_and_get_date(self):
        ...
        # 更新
        update_last_question_date(ticker, test_date)

        # 获取
        retrieved_date = get_last_question_date(ticker)
...
class TestModuleConstants(unittest.TestCase):
    ...
    def test_headers_defined(self):
        """测试请求头定义"""
        from standalone_ehd_downloader import HEADERS
